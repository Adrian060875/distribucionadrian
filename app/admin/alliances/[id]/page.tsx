...
export default function AllianceCommissionsPage() {
  ...
  async function load() {
    ...
    json.orders.forEach(o => {
      const saldoPesos = Math.max(0, Math.floor((o.balance ?? Math.max(o.due - o.paid, 0)) / 100));
      init[o.orderId] = { amount: String(saldoPesos), date: today, notes: "" };
    });
    setForms(init);
  }
  ...
  async function pay(orderId: string) {
    ...
    const amtPesos = parseInt(f?.amount || "0", 10) || 0; // UI en pesos
    if (!amtPesos || amtPesos <= 0) { alert("Importe inválido"); return; }

    const ord = data.orders.find(o=>o.orderId===orderId);
    const saldoPesosMax = ord ? Math.max(0, Math.floor((ord.balance ?? Math.max(ord.due - ord.paid,0))/100)) : amtPesos;
    ...
    const r = await fetch("/api/commission-payments", {
      ...
      body: JSON.stringify({
        ...
        amount: safePesos * 100,            // a centavos
  ...
  // Totales CORREGIDOS en cliente (centavos)
  const totals = useMemo(() => {
    if (!data) return { due: 0, paid: 0, balance: 0 };
    const due  = data.orders.reduce((a,o)=>a + Math.max(o.due,0), 0);
    const paid = data.orders.reduce((a,o)=>a + Math.max(o.paid,0), 0);
    ...
    <div className="p-6 grid gap-6">
      <div className="flex items-center justify-between">
        ...
        <div className="card px-4 py-2">
          ...
          <div className="text-lg font-bold text-amber-300">{money.format(totals.balance/100)}</div>
        </div>
      </div>
      ...
      <section className="card p-5 grid gap-4">
        <div className="grid gap-1 sm:grid-cols-3 lg:grid-cols-6 bg-slate-900/40 rounded-lg p-3">
          ...
          <div className="flex justify-between"><span>Total devengado</span><b>{money.format(totals.due/100)}</b></div>
          <div className="flex justify-between"><span>Pagado</span><b className="text-emerald-300">{money.format(totals.paid/100)}</b></div>
          <div className="flex justify-between"><span>Saldo</span><b className="text-amber-300">{money.format(totals.balance/100)}</b></div>
        </div>
            ...
            <tbody>
              {data.orders.map(o => (
                <tr key={o.orderId}>
                  ...
                  <td className="text-right text-amber-300">{money.format(Math.max(o.due - o.paid,0)/100)}</td>
