...
export default function PlansPage() {
  ...
  async function del(id: string) {
    if (!confirm("Â¿Borrar plan de financiaciÃ³n? Si estÃ¡ usado por Ã³rdenes, puede fallar.")) return;
    const r = await fetch(`/api/financing-plans/${id}`, { method: "DELETE" });
    if (!r.ok) {
      const t = await r.text().catch(() => "");
      alert(t || "No se pudo borrar (Â¿estÃ¡ referenciado por alguna orden?)");
      return;
    }
    load();
  }
  ...
  return (
    <div className="space-y-6 p-6">
      <div className="flex items-center gap-3">
        <a href="/admin" className="rounded-lg border border-emerald-700/40 px-3 py-1.5 text-emerald-300 hover:bg-emerald-700/10">â† Volver</a>
        <h1 className="text-2xl font-semibold">Planes de FinanciaciÃ³n</h1>
      </div>
      ...
      <form onSubmit={createPlan} className="grid md:grid-cols-6 gap-3 bg-zinc-900/60 p-4 rounded-2xl border border-zinc-800">
        ...
        <div>
          <div className="text-xs text-zinc-400">% InterÃ©s</div>
        ...
        <div className="md:col-span-2 text-right">
          <button disabled={savingNew} className="btn-emerald">{savingNew ? "Guardandoâ€¦" : "Agregar plan"}</button>
        </div>
      </form>
        ...
        <table className="w-full text-sm">
          <thead className="text-zinc-400">
            <tr>
              ...
              <th className="p-2 text-right">% InterÃ©s</th>
