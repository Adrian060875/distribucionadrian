...
type Payload = { seller: { id: string; name: string; commissionPct: number }, orders: Row[], totals: { due: number; paid: number; balance: number } };
...
export default function SellerCommissionsPage() {
  ...
  async function load() {
    if (!id) return;
    setLoading(true);
    const r = await fetch(`/api/commissions/sellers/${id}`, { cache: "no-store" });
    setLoading(false);
    if (!r.ok) { alert("No se pudo cargar"); router.push("/admin/sellers"); return; }
  ...
  async function pay(orderId: string) {
    ...
    const amt = parseInt(f?.amount || "0", 10) || 0;
  ...
  return (
    <div className="p-6 grid gap-6">
      <div className="flex items-center justify-between">
        <a className="btn btn-ghost" href="/admin/sellers">← Vendedores</a>
        <h1 className="text-xl font-semibold">Comisiones — {data.seller.name}</h1>
      ...
      <section className="card p-5 grid gap-4">
        <div className="grid gap-1 sm:grid-cols-3 lg:grid-cols-6 bg-slate-900/40 rounded-lg p-3">
          <div className="flex justify-between"><span>% Comisión</span><b>{data.seller.commissionPct}%</b></div>
